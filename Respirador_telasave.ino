...
#include "Arduino.h"
#include <TaskScheduler.h>  ///era v 3.2.2
...
void FLASHvariables::initialize()    
{
 ...
 pressaomax=45; 
 pressaomin=30;
 volumecorrentemax=422;
 volumecorrentemin=222;
 peepmax=7;
 peepmin=4;
 fluxomax=38;
 fluxomin=20;
...
{
    vci.getValue(&myVars.volumecorrentemin);
    myVars.volumecorrentemin += 1;
    vci.setValue(myVars.volumecorrentemin);
}
...
{
    vca.getValue(&myVars.volumecorrentemax);
    myVars.volumecorrentemax += 1;
    vca.setValue(myVars.volumecorrentemax);
}
...
{
    fli.getValue(&myVars.fluxomin);
    myVars.fluxomin += 1;
    fli.setValue(myVars.fluxomin);
}
...
{
    fla.getValue(&myVars.fluxomax);
    myVars.fluxomax += 1;
    fla.setValue(myVars.fluxomax);
}
...
{
    pri.getValue(&myVars.pressaomin);
    myVars.pressaomin += 1;
    pri.setValue(myVars.pressaomin);
}
...
{
    pra.getValue(&myVars.pressaomax);
    myVars.pressaomax += 1;
    pra.setValue(myVars.pressaomax);
}
...
{
    pei.getValue(&myVars.peepmin);
    myVars.peepmin += 1;
    pei.setValue(myVars.peepmin);
}
...
{
    pea.getValue(&myVars.peepmax);
    myVars.peepmax += 1;
    pea.setValue(myVars.peepmax);
}
...
{
    pti.getValue(&myVars.platomin);
    myVars.platomin += 1;
    pti.setValue(myVars.platomin);
}
...
{
    pta.getValue(&myVars.platomax);
    myVars.platomax += 1;
    pta.setValue(myVars.platomax);
}
...
{
    rpi.getValue(&myVars.rpmmin);
    myVars.rpmmin += 1;
    rpi.setValue(myVars.rpmmin);
}
...
{
    rpa.getValue(&myVars.rpmmax);
    myVars.rpmmax += 1;
    rpa.setValue(myVars.rpmmax);
}
...
{
    api.getValue(&myVars.minapnea);
    myVars.minapnea += 1;
    api.setValue(myVars.minapnea);
}
...
{
    apa.getValue(&myVars.maxapnea);
    myVars.maxapnea += 1;
    apa.setValue(myVars.maxapnea);
}
...
{
    soi.getValue(&myVars.sao2min);
    myVars.sao2min += 1;
    soi.setValue(myVars.sao2min);
}
...
{
   coi.getValue(&myVars.co2min);
    myVars.co2min += 1;
    coi.setValue(myVars.co2min);
}
...
if(dual_state==1){
  sexo=true;
  c2.setValue(1);
  if (sexo==true)myVars.volcalc= (52 + (0.75 * (myVars.altura - 152.4)))*6;
if (sexo==false)myVars.volcalc= (49 + (0.67 * (myVars.altura - 152.4)))*6;
n6.setValue(myVars.volcalc);
}
  else{
    c2.setValue(0);
    sexo=false;
...
if (myVars.sense_state != 1){
  myVars.sense_state=1;
}
if(myVars.sense_state==1)   {
      bt3.setValue(1);
      bt4.setValue(0);
...
if (myVars.sense_state != 2){
  myVars.sense_state=2;
}
if(myVars.sense_state==2){
      bt4.setValue(1);
      bt3.setValue(0);
...
{
    n0.getValue(&myVars.volume);
    myVars.volume += 1;
    n0.setValue(myVars.volume);
}
...
{
    n1.getValue(&myVars.RPM);
    myVars.RPM += 1;
    n1.setValue(myVars.RPM);
}
...
{
    n22.getValue(&myVars.pressao);
    myVars.pressao += 1;
    n22.setValue(myVars.pressao);
}
...
{
    fl.getValue(&myVars.fluxs);
    myVars.fluxs += 1;
    fl.setValue(myVars.fluxs);
}
...
{
    pe.getValue(&myVars.PEEP);
    myVars.PEEP += 1;
    pe.setValue(myVars.PEEP);
}
...
{
    n4.getValue(&myVars.FIO2);
    myVars.FIO2 += 1;
    n4.setValue(myVars.FIO2);
}
...
{
    x2.getValue(&myVars.SENS);
    myVars.SENS += 1;
    x2.setValue(myVars.SENS);
}
...
{
    x3.getValue(&myVars.TINS);
    myVars.TINS += 1;
    x3.setValue(myVars.TINS);
}
...
{
    x4.getValue(&myVars.TSUB);
    myVars.TSUB += 1;
    x4.setValue(myVars.TSUB);
}
...
{
    n7.getValue(&myVars.INSPEXP);
    myVars.INSPEXP += 1;
    n7.setValue(myVars.INSPEXP);
}
...
{
    x0.getValue(&myVars.pausa);
    myVars.pausa += 1;
    x0.setValue(myVars.pausa);
}
...
{
    x5.getValue(&myVars.altura);
    myVars.altura += 1;
    x5.setValue(myVars.altura);
...
{
    n5.getValue(&myVars.idade);
    myVars.idade += 1;
    n5.setValue(myVars.idade);
}
...
if(pag==2&&cont>=20){
 ...
 Serial2.write(0xff);
 Serial2.write(0xff);
if(volumecorrente>=myVars.volumecorrentemax || volumecorrente<=myVars.volumecorrentemin)a2.Set_background_color_bco(63488);

if(fluxoinstantaneo>myVars.fluxomax || fluxoinstantaneo<myVars.fluxomin) a3.Set_background_color_bco(63488); 

if(pressaoinstantanea>myVars.pressaomax || pressaoinstantanea<myVars.pressaomin) a4.Set_background_color_bco(63488); 

if(pressaopk>myVars.pressaomax)a4.Set_background_color_bco(63488); 

if(lastpeep>myVars.peepmax || lastpeep<myVars.peepmin)  a5.Set_background_color_bco(63488); 

if (plato > myVars.platomax || plato < myVars.platomin)  a6.Set_background_color_bco(63488); 

if(rpmpaciente>myVars.rpmmax || rpmpaciente< myVars.rpmmin) a7.Set_background_color_bco(63488); 
...
void t3Callback() {
  ...
  tempoexpiracao = 600/myVars.RPM*(100-myVars.INSPEXP);
tempoinspiracao = 600/myVars.RPM*myVars.INSPEXP;
//Serial.println("start");
  if (myVars.mode_state == 1){//vcv
  ...
  if (tempo<=15){
    ...
    if(volumecorrente<=myVars.volume){
      servo1.write(10 );//servos ins abre max 
     // Pause=Pause+20;
    }
     //if (Pause >= pausaexp){
  if ((volumecorrente>=myVars.volume)||(tempo >= tempoinspiracao)){ //|| tempo >= tempoinspiracao){
...
if (ciclo == false){          // false = expiracao 
 ...
 //Pause=Pause+20;
if (pressaoinstantanea <= myVars.PEEP){
...
if (myVars.mode_state == 2){//pcv

if (ciclo == true && pressaoinstantanea <= myVars.pressao){  // true = inspiracao
  tempo =tempo+10;
  servo1.write(10 );
  //Pause=Pause+20;
if (pressaoinstantanea >= myVars.pressao){ 
    servo2.write(45);
...
//pausa
   ...
   //altura
    ...
    b27.attachPop(b27PopCallback);
     b16.attachPop(b16PopCallback);//save
